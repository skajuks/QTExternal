...
namespace D3D9
{
    ...
    void D3D9Render::String( int x, int y, Color color, ID3DXFont* pFont, const char *fmt, ... )
    {
        char Buffer[ 4048 ];
        va_list vaList;
        va_start( vaList, fmt );
    ...
    void D3D9Render::StringOutlined( int x, int y, Color color, ID3DXFont* pFont, const char *fmt, ... )
    {
        char Buffer[ 4048 ];
        va_list vaList;
        va_start( vaList, fmt );
    ...
    void D3D9Render::Rect( int x, int y, int l, int h, Color color )
    {
        D3DRECT rect = { x, y, x + l, y + h };
        pDevice->Clear( 1, &rect, D3DCLEAR_TARGET, D3DCOLOR_ARGB( color.a(), color.r(), color.g(), color.b() ), 0, 0 );
    }
    void D3D9Render::BorderBox( int x, int y, int l, int h, int thickness, Color color )
    {
    ...
    void D3D9Render::BorderBoxOutlined( int x, int y, int l, int h, int thickness, Color color, Color outlined )
    {
    ...
    void D3D9Render::RectOutlined( int x, int y, int l, int h, Color rectcolor, Color outlinedcolor, int thickness )
    {
        Rect( x, y, l, h, rectcolor );
        BorderBox( x - thickness, y - thickness, l + thickness, h + thickness, thickness, outlinedcolor );
    }
    void D3D9Render::Line( int x, int y, int x2, int y2, Color color, float thickness )
    {
    ...
    void D3D9Render::GardientRect( int x, int y, int w, int h, int thickness, bool outlined, Color from, Color to, Color Outlined )
    {
    ...
    HRESULT Startup( HWND hWindow )
    {
        ...
        if( !AddFont( "Arial", "Arial", 14, 0, FW_NORMAL, false ) )
            return E_FAIL;
    ...
    bool Render( void )
    {
        ...
        if( SUCCEEDED( pDevice->BeginScene() ) )
        {
            ...
            {
                pRender->Line(10,10,800,800, Color(255,255,0), 10);
                pRender->String( 5, 5, Color( 255, 255, 255 ), GetFont( "Arial" ), "sdfsdfsdfsdfsdfdsfsdfdsfdsfs" );
            }
            pDevice->EndScene();
        }
    ...
    HRESULT CreateOverlay( std::string szGameWindowTitle, std::string szOverlayTitle )
    {
        ...
        WNDCLASSEX wc	= {	sizeof( WNDCLASSEX ), 0, WndProc, 0, 0, NULL, LoadIcon( NULL, IDI_APPLICATION ),
                            LoadCursor( 0, IDC_ARROW ), 0, 0, ( LPCWSTR )szOverlayTitle.c_str(), LoadIcon( 0, IDI_APPLICATION ) };
        ...
        hWndOverlay		= CreateWindowEx( WS_EX_TOPMOST | WS_EX_COMPOSITED | WS_EX_TRANSPARENT | WS_EX_LAYERED, ( LPCWSTR )szOverlayTitle.c_str(), ( LPCWSTR )szOverlayTitle.c_str(),
